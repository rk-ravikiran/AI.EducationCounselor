...
def display_programs(data):
    ...
    for i, prog in enumerate(programs[:5], 1):
        ...
        with st.expander(f"#{i} {badge} - {prog.get('program', 'N/A')} ({score:.0%} fit)", expanded=expanded):
            col1, col2 = st.columns([2, 1])
            
            with col1:
                st.markdown(f"**🏫 Institution:** {prog.get('institution', 'N/A')}")
                st.markdown(f"**📚 Level:** {prog.get('level', 'N/A')}")
                
                # Show AI reasoning
                if expl.get("reason"):
                    st.markdown("**🤖 AI Analysis:**")
                    st.info(expl.get("reason"))
                
                # Show what student should know
                if expl.get("student_note"):
                    st.markdown("**� Important for You:**")
                    st.warning(expl.get("student_note"))
                
                # Show URL
                if prog.get("url"):
                    st.markdown(f"**� Program Page:** [{prog.get('url')}]({prog.get('url')})")
            
            with col2:
                st.metric("Fit Score", f"{score:.0%}")
                
                # Matched interests
                matched = expl.get("matched_keywords", [])
                if matched:
                ...
                # Matched strengths
                strengths = expl.get("matched_strengths", [])
                if strengths:
                ...
                if expl.get("data_source") == "ai_reasoning":
...
if run_btn:
    if not selected_interests:
        ...
        # Display agent outputs in organized tabs
        ...
        for agent_name, payload in result["agents"].items():
            tab_idx = agent_mapping.get(agent_name, 0)
            with agent_tabs[tab_idx]:
                # Use custom display function if available, otherwise show JSON
                display_func = agent_display_functions.get(agent_name)
                if display_func:
                    # For institutional_data, ensure we only pass program_suggestions
                    if agent_name == "institutional_data" and isinstance(payload, dict):
                        # Filter out any non-program data
                        clean_payload = {"program_suggestions": payload.get("program_suggestions", [])}
                        display_func(clean_payload)
                    else:
